...
import { getContentFromDirectoryPath, TargetsApiResponse, DirectoryPath } from '../helpers/server/fileApi'
...
const readTargetDirectory: ServerMiddleware = function (req, res) {
  ...
  req.on('data', (chunk) => {
    ...
    if (bodyIsValid(body)) {
      getContentFromDirectoryPath(body.directoryPath, runtimeConfiguration.extentionMapping).then((resp) => {
        res.setHeader('Content-Type', 'application/json')
        res.end(JSON.stringify(resp))
      }).catch(() => {
        res.statusCode = 404
        res.statusMessage = 'Fuck you looser but my code is probably at fault and not the config'
        res.end()
      })
    } else {
      res.statusCode = 404
