...
export type ExecTargetConfigElem = {
...
export function filterOutUnvalidExtentionFiles(
  fileNameArray: FileName[],
  extentionMapping: ExtentionMapping
): FileName[] {
  ...
  return fileNameArray.filter((fileName) => {
    ...
    return (
      (extention === null || validExtentionList.includes(extention)) &&
      !fileName.startsWith(".")
    )
  })
}

export function extractExtentionFromFileName(
  fileName: FileName
): FileName | null {
...
export function getTargetFileListFromDirectoryPath(
  directoryPath: DirectoryPath
): Promise<FileName[]> {
  return new Promise((resolve, reject) => {
    fs.readdir(
